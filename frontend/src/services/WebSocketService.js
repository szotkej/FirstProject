...
class WebSocketService {
    ...
    INACTIVITY_LIMIT = 20 * 60 * 1000; // 15 minuta
    localStatus = 'Online'; // Lokalny status dla każdej karty
    lastSentStatus = 'Online'; // Ostatni wysłany status (tylko dla master)
    ...
    becomeMaster() {
        ...
        this.socket.addEventListener('open', () => {
            ...
            this.pendingSubscriptions.forEach(ch => this.sendRaw({ subscribe: { channel: ch }, id: Date.now() }));
    ...
    connect({ user_id, username, credentials, }) {
